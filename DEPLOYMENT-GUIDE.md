...
### 7.6 Open the app in your browser
```powershell
kubectl get service frontend-service
```
Wait until `EXTERNAL-IP` shows a real IP (not `<pending>`) — takes 1–2 minutes.

Open `http://EXTERNAL-IP` in your browser.
You will see the **red "BEFORE OPTIMIZATION"** page showing the waste:
10 replicas, 2Gi/1000m per pod, LoadBalancers per service, ~$2,625/mo.

To see the backend page:
```powershell
kubectl get service backend-api-service
```
Open `http://BACKEND-EXTERNAL-IP` in your browser.
...
### 9.7 Open the optimized app in your browser
```powershell
kubectl get service -n ingress-nginx
```
Copy the `EXTERNAL-IP` of `ingress-nginx-controller`.

Open `http://INGRESS-EXTERNAL-IP` — you will see the **green "AFTER OPTIMIZATION"** page:
2 replicas, 300Mi/150m per pod, single Ingress, ~$348/mo, saving 87%.

Open `http://INGRESS-EXTERNAL-IP/api` to see the backend API page.
